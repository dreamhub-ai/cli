...
def list_activities(
    ...
    response = client.post(f"{resource}/{entity_id}/activities/fetch", json_payload=payload, retry_on_401=True)
    data = _ok(response)
...
def get_activity(entity_type: str, entity_id: str, activity_id: str, size: int = 500) -> dict:
    ...
    resource = _resolve_entity_resource(entity_type)
    client = _client()
    response = client.post(f"{resource}/{entity_id}/activities/fetch", json_payload={"size": size}, retry_on_401=True)
    data = _ok(response)
    if "error" in data:
        return data
    for activity in data.get("activities", []):
...
def search(
    ...
    response = client.post("search/global", json_payload=payload, retry_on_401=True)
    return _ok(response)
